...
% Zoom/Pan
% hZoomMenuitem = uimenu(...       
%     'Parent',hDisplayMenu,...
%     'Label','Zoom mode',...
%     'HandleVisibility','callback', ...
%     'Callback', @hZoomMenuitemCallback);
% 
% hPanMenuitem = uimenu(...       
%     'Parent',hDisplayMenu,...
%     'Label','Pan mode',...
%     'HandleVisibility','callback', ...
%     'Callback', @hPanMenuitemCallback);
...
% 
% % Prolifes 
% hExtractProfilesMenuitem = uimenu(...       
%     'Parent',hPIVPostprocessMenu,...
%     'Label','Extract profile',...
%     'HandleVisibility','callback', ...
%     'Callback', @hExtractProfilesMenuitemCallback);
% 
% % Stats 
% hExtractStatsMenuitem = uimenu(...       
%     'Parent',hPIVPostprocessMenu,...
%     'Label','Extract statistics',...
%     'HandleVisibility','callback', ...
%     'Callback', @hExtractStatisticsMenuitemCallback);
...
% Create Play, Stop, slider, ImNumber and time
% hplay = uicontrol(...
%     'Callback', @hPlayCallback,...
%     'Style','pushbutton',...
%     'String','Play',...
%     'Parent', hpanelPlayer, ...
%     'Units', 'normalized', ...
%     'Position',[0.15 0.20 0.05 0.75]);
% 
% hstop = uicontrol(...
%     'Callback', @hStopCallback,...
%     'Style','pushbutton', ...
%     'String','Stop',...
%     'Parent', hpanelPlayer, ...
%     'Units', 'normalized', ...
%     'Position',[0.20 0.20 0.05 0.75]);
...
% hpopupStepPass1 = uicontrol(...
%     'Style','popupmenu',...
%     'String',{'512', '256','128','64','32','16','8'},...
%     'Parent', hpanelPIVSettings, ...
%     'Units', 'normalized', ...
...
hpopupVecDerivativeTypeSelector = uicontrol(...
    'Style','popupmenu',...
    'String',{'Exx','Eyy','Exy','Eyx','vorticity','divergence','V','Theta','Vx','Vy'},...
    'Parent', hpanelChangeVectorDerivativeDisplaySettings, ...
    'Units', 'normalized', ...
...
%DirEntries = getAllFiles(CPT_Folder)
%NewListCPT=char({DirEntries.name})
%whos NewListCPT
...
hStartExportAllButton = uicontrol(...
    ...
    'Position',[0.6 0.7 0.1 0.05]);
% %
% hpanelEulerianSum = uipanel(...
%     'Title','Calculate Eulerian sum',...
%     'FontSize',8,...
%     'Parent', hSecondFigure, ...
%     'Units', 'normalized', ...
%     'Position',[0.01 0.01 0.98 0.98]);
...
%     function hControlPointsExtractionDoneCallback(hDoneControlPointsExtraction,eventdata)
%         % This function and button are not needed. REMOVE
%         hpanelGetControlPoints.Visible = 'off';
%         hSecondFigure.Visible = 'off';
%         guidata(hMainFigure,myhandles);
%     end
...
%             myhandles.RawCpt{1,4}='phys';
%             myhandles.RawCpt{1,5}=ImScale;
            % Display the first image
            Frame=1;
...
%        myhandles.Datasets{ThisDataSetNumber,9} = XPix;
 %       myhandles.Datasets{ThisDataSetNumber,10} = YPix;
        ...
        myhandles.Datasets{ThisDataSetNumber,11} = ImScale;
        myhandles.Datasets{ThisDataSetNumber,12} = 'phys'; % toggle scalebar on/off
        myhandles.Datasets{ThisDataSetNumber,13} = myhandles.PhysU;
        
        myhandles.RawCpt{1,4}=myhandles.Datasets{ThisDataSetNumber,12}; %togle scalebar on/off
        myhandles.RawCpt{1,5}=myhandles.Datasets{ThisDataSetNumber,11}; % ImScale
        myhandles.RawCpt{1,6}= myhandles.Datasets{ThisDataSetNumber,13}; % physical unit
...
%         end
    ...
    function hStartUndeformCallback (hStartUndeform,eventdata)
        ...
        DataFolder=myhandles.DataSets{DataSetNumber,1}; % Cam_1/Raw or Cam_2/Raw

        cd( DataFolder ); %go into Cam_1/Raw
        mkdir( 'Rectified' );  % make folder Cam_1/Raw/Rectified
    ...
    function hApplyDisplaySetting(hApplyDisplaySettingButton,eventdata)
        ...
        if isempty(k) == 1 % Dataset is not vector
            ...
            if isempty(k) == 1 % Dataset includes Vector but not Rectified
                ...
                myhandles.VectorField{1,7} = U; %cell2mat(U); 
                myhandles.VectorField{1,8} = V; %cell2mat(V);
                %myhandles.VectorField{1,10} = typevector; %cell2mat(V);
                ...
                myhandles.VectorField{1,7} = U; %cell2mat(U); 
                myhandles.VectorField{1,8} = V; %cell2mat(V); 
                %myhandles.VectorField{1,10} = typevector;
