function TecPIV_ExtractGCP(TecPIVFolder, DataSets,DataSetNumber,Ax,RawCpt,n_sq_x,n_sq_y,dX,dY,VectorField,Derivative)
...
axes(Ax);  % select the axe
TecPIV_Display_2(Ax,1,DataSets,DataSetNumber,RawCpt,VectorField,Derivative);
drawnow; hold on;
PIVStagePath=DataSets{DataSetNumber,2};
ProjectName=DataSets{DataSetNumber,3};
ValueDataImg = DataSets{DataSetNumber,11}; % number of the dataset containing the images
DataFolder = DataSets{DataSetNumber,1}
ImgPath = DataSets{ValueDataImg,1};
F = 1; % use first image (only image)
...
SaveFile=fullfile(PIVStagePath,ProjectName,DataFolder,'CalibrationPoints.mat');
...
message=sprintf('%d control points found in calibration image\n',Np);
disp(message);
